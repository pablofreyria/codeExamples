...
deaths_sl1 <- prediction_sl_origin(total_deaths,outname = outname,varnames = varnames)

cases_sl2 <- prediction_sl_window(total_cases,outname = outname,varnames = varnames)
deaths_sl2 <- prediction_sl_window(total_deaths,outname = outname,varnames = varnames)


# cross validate the Super Learner
cases_sl_cv <- cross_validate_SL(total_cases,cases_sl1$SuperLearner,
                                 outname,varnames)
death_sl_cv <- cross_validate_SL(total_deaths,deaths_sl1$SuperLearner,
                                 outname,varnames)

cases_sl_cv2 <- cross_validate_SL(total_cases,cases_sl2$SuperLearner,
                                 outname,varnames)
death_sl_cv2 <- cross_validate_SL(total_deaths,deaths_sl2$SuperLearner,
                                 outname,varnames)



# MSE standarized by mean of predictions
## MSE is different than the risk estimate
mse_cases <- mean((cases_sl2$Preds-total_cases$TotalCases)^2)/
              mean(total_cases$TotalCases)
mse_deaths <- mean((deaths_sl1$Preds-total_deaths$TotalCases)^2)/
  mean(total_deaths$TotalCases)


# Plots the fits
sl_cases_plot  <- data.frame(Prediction = cases_sl2$Preds,Real=total_cases$TotalCases,
                             Date = as.Date(total_cases$DateNum))
sl_deaths_plot <- data.frame(Prediction = deaths_sl2$Preds,Real=total_deaths$TotalCases,
