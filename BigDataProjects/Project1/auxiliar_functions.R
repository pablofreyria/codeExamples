...
#Function to change from list to a big data frame, by MemberID & Year
...
# method to build the final tables with main and square terms
...
mahalanobis_clases <- function(df,var_name){
  
  id_var <- which(colnames(df)==var_name)
  
  values <- unique(df[,id_var])
  n_val  <- length(values)
  
  clas_defs <- list(Means = matrix(NA,nrow = (ncol(df)-1),ncol = n_val),
                    Vars = vector(mode="list",n_val),Values = values,Category = var_name)
  rownames(clas_defs$Means) <- colnames(df)[-id_var]
  
  for(i in c(1:n_val)){
    # id_var is constant across each class
    clas_defs$Means[,i] <- t(colMeans(df[df[,id_var]==values[i],-id_var]))
    clas_defs$Vars[[i]]  <- cov(df[df[,id_var]==values[i],-id_var])
  }
...
# return the class of a data frame based on a "mahalanobis_clases" object
mahalanobis_classify <- function(df,maha_clas){
  
  vals <- maha_clas$Values
  id_class <- which(colnames(df)==maha_clas$Category)
  names_varval <- paste(maha_clas$Category,maha_clas$Values,sep = "")
  
  predictions <- matrix(NA,nrow = nrow(df),length(names_varval))
  colnames(predictions) <- names_varval
 
   for (i in c(1:length(names_varval))){
    predictions[,i] <-  mahalanobis(df[,-id_class],maha_clas$Means[i],maha_clas$Vars[[i]])
  }
