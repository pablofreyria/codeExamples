# Calculo de correlaci?n entre efectos estimados de Jones
library("reshape2")  
library("ggplot2")
#source('~/TESIS2/R/dise?oPorFactnum.R')
source('~/Documents/Tesis/Latex/R/dise§oPorFactnum.R')
  ...
  for(factNum in c(1:length(facts))){
    # leer matriz de dise?o y de experimento
    m <- facts[factNum]
    mat<- matrizDisenoDef(m)  # Dise?os dados por Jones
    #mat <- disenoJones(m)      # Dise?os de R
    ...
    cor_qqst_avg <- 0
      cor_stuv_max <- 0
      cor_stuv_avg <- 0
    # iterar sobre todas las formas de elegir 3 par?metros de m
    for(i in c(1:choose(m,3))){
      # hacer la matriz de dise?o y de experimento a partir de la de tres factores
      ...
      # calcular la correlacion por tipo, la matriz tiene la misma forma->ids fijos
        ...
        cor_qqss_avg <-sum(cor_qqss_avg,abs(matcors[id_qqss]))
      
        # entre cuadr?ticos interaccion factor  comun
        id_qqqs <- c(75,85,76,96,87,97)
        cor_qqqs_max <-max(cor_qqqs_max,matcors[id_qqqs])
        cor_qqqs_avg <-sum(cor_qqqs_avg,abs(matcors[id_qqqs]))
      
        # entre cuadr?ticos interaccion factor no comun
        id_qqst <- c(95,86,77)
        cor_qqst_max <-max(cor_qqst_max,matcors[id_qqst])
        cor_qqst_avg <-sum(cor_qqst_avg,abs(matcors[id_qqst]))
        
        # entre interacciones
        id_stuv <- c(79,80,90)
        cor_stuv_max <-max(cor_stuv_max,matcors[id_stuv])
        cor_stuv_avg <-sum(cor_stuv_avg,abs(matcors[id_stuv]))
    }
    # poner los valores en la tabla por n?mero de par?metro
 ...
 # plot_corr <- melt(corrEstimadas,id.vars = 1)
  ...
  fig1<-ggplot(plot_corr_max,aes(x=m,y=value,group=variable,color=variable))+
    ...
    # scale_shape_manual(labels=c('Cuadr?ticos','Cuad-inter com?n','Cuad-inter no com?n',
    #                             'Inter-inter com?n','Inter-inter no com?n'),
    #                    values = c('black','blue4','darkred','palegreen4','orangered4'))
    labs(x="N?mero de factores", y="Corr. Max estimadores",colour=" ",shape=' ',shape_size=" ")+
